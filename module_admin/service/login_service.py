...
class LoginService:
    ...
    async def register_user_services(
        ...
        print('zwl-->>11', register_enabled, RedisInitKeyConfig.SYS_CONFIG.key)
        captcha_enabled = (
            await request.app.state.redis.get(f'{RedisInitKeyConfig.SYS_CONFIG.key}:sys.account.captchaEnabled')
            == 'true'
        )
        print('zwl-->>22', captcha_enabled, RedisInitKeyConfig.SYS_CONFIG.key)
